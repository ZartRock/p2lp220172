...
public abstract class PratoPersonalizado implements Comparable<PratoPersonalizado> {
	...
	public double calculaPrecoFinal() {
		return getPrecoBase() + (0.01 * getAreaTotal());
	}

	public abstract double getAreaTotal();
	
	public int compareTo(PratoPersonalizado outroPrato){
		if(this.calculaPrecoFinal() < outroPrato.calculaPrecoFinal()){
			return -1;			
		}else if(this.calculaPrecoFinal() == outroPrato.calculaPrecoFinal()){
			return 0;
		}else{
			return 1;
		}
